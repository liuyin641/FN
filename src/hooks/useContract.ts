...
import BODE_SALE_ABI from '../constants/abis/node_sale.json'
...
import EARN_ABI from '../constants/abis/earn.json'
import LIVE_EARN_ABI from '../constants/abis/live-earn.json'
import NODE_REWARDS_ABI from '../constants/abis/node-reward.json'
import { EARN_ADDRESS, LIVE_EARN_ADDRESS, NODE_DIVIDEND_ADDRESS, NODE_SALE_ADDRESS } from '../constants'
...
export function useNodeContract(): Contract | null {
  const { chainId } = useActiveWeb3React()
  return useContract(NODE_SALE_ADDRESS[chainId ?? 56], BODE_SALE_ABI, true)
}

export function useEarnContract({ isLive }: { isLive: boolean }): Contract | null {
  const { chainId } = useActiveWeb3React()
  return useContract(
    isLive ? LIVE_EARN_ADDRESS[chainId ?? 56] : EARN_ADDRESS[chainId ?? 56],
    isLive ? LIVE_EARN_ABI : EARN_ABI,
    true
  )
}

export function useNodeRewardsContract(): Contract | null {
  const { chainId } = useActiveWeb3React()
  return useContract(NODE_DIVIDEND_ADDRESS[chainId ?? 56], NODE_REWARDS_ABI, true)
}
